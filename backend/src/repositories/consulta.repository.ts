...
export class ConsultaRepository {
  async create(data: ConsultaInsert): Promise<Consulta> {
    try {
      const processedData = {
        ...
        dataHoraInicio: data.dataHoraInicio ? new Date(data.dataHoraInicio) : null,
        dataHoraFim: data.dataHoraFim ? new Date(data.dataHoraFim) : null,
      };
      
      const [result] = await db.insert(consulta).values(processedData).returning();
  ...
  async findById(dataHoraAgendada: Date, cpfFuncSaude: string, cpfPaciente: string): Promise<Consulta | null> {
